...
def run_mininet(manifest):
    ...
    with open(message_file, "w") as message:
        ...
        print(file=message)
    #        print('To run the switch debugger,
    # run this command from your host OS:', file=message)
    ...
    if "num-hosts" in manifest.target_config:
        switch_args.append(
            "--num-hosts %s" % manifest.target_config["num-hosts"]
        )
...
def run_stf(manifest):
    ...
    if "test" not in manifest.target_config:
...
def run_custom(manifest):
    ...
    rv = run_command(
        "%s python3 %s %s" % (python_path, program, " ".join(script_args))
    )
