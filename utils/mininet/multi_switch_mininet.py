...
def main():
    ...
    os.environ.update(
        dict([(k_v[0], str(k_v[1])) for k_v in iter(params.items())])
    )

    def formatParams(s):
        for param in params:
            s = re.sub(
                r"\$" + param + r"(\W|$)", str(params[param]) + r"\1", s
            )
    ...
    links = [link[:2] for link in conf["links"]]
    latencies = dict(
        [
            ("".join(sorted(latency[:2])), latency[2])
            for latency in conf["links"]
            if len(latency) >= 3
        ]
    )
    bws = dict(
        [
            ("".join(sorted(latency[:2])), latency[3])
            for latency in conf["links"]
            if len(latency) >= 4
        ]
    )
    ...
    for latency in latencies:
        if isinstance(latencies[latency], str):
            latencies[latency] = formatParams(latencies[latency])
        else:
            latencies[latency] = str(latencies[latency]) + "ms"
    ...
    print(
        "\n".join(
            ["%s: %s" % (k_v1[0], k_v1[1]) for k_v1 in iter(params.items())]
        )
        + "\n"
    )
    ...
    for p, host_name in host_procs:
        if (
            "wait" in conf["hosts"][host_name]
            and conf["hosts"][host_name]["wait"]
        ):
            _wait_for_exit(p, host_name)
    ...
    if "after" in conf and "cmd" in conf["after"]:
        cmds = (
            conf["after"]["cmd"]
            if type(conf["after"]["cmd"]) is list
            else [conf["after"]["cmd"]]
        )
        for cmd in cmds:
            os.system(cmd)
