...
class AppController:
    ...
    def __init__(
        self, manifest=None, target=None, topo=None, net=None, links=None
    ):
    ...
    def read_register(self, register, idx, thrift_port=9090, sw=None):
        ...
        reg_val = [
            r for r in stdout.split("\n") if " %s[%d]" % (register, idx) in r
        ][0].split("= ", 1)[1]
        return int(reg_val)
    ...
    def start(self):
        ...
        for sw in self.topo.switches():
            ...
            if (
                ...
                if type(extra_entries) is list:  # array of entries
        ...
        for host_name in self.topo._host_links:
            ...
            for link in list(self.topo._host_links[host_name].values()):
                sw = link["sw"]
                # entries[sw].append('table_add send_frame rewrite_mac %d =>
                # %s' % (link['sw_port'], link['sw_mac']))
                # entries[sw].append('table_add forward set_dmac %s =>
                # %s' % (link['host_ip'], link['host_mac']))
                # entries[sw].append('table_add ipv4_lpm set_nhop %s/32 =>
                # %s %d' % (link['host_ip'], link['host_ip'], link['sw_port']))
                ...
                # h.cmd('ifconfig %s %s hw ether %s' % (iface, link['host_ip'],
                # link['host_mac']))
                h.cmd(
                    "arp -i %s -s %s %s"
        ...
        for h in self.net.hosts:
            ...
            for sw in self.net.switches:
                path = shortestpath.get(
                    sw.name, h.name, exclude=lambda n: n[0] == "h"
                )
                if not path:
                    continue
                if not path[1][0] == "s":
                ...
                # entries[sw.name].append('table_add send_frame rewrite_mac %d
                # => %s' % (sw_link[0]['port'], sw_link[0]['mac']))
                # entries[sw.name].append('table_add forward set_dmac %s
                # => %s' % (h_link['host_ip'], sw_link[1]['mac']))
                # entries[sw.name].append('table_add ipv4_lpm set_nhop %s/32
                # => %s %d' % (h_link['host_ip'], h_link['host_ip'],
                # sw_link[0]['port']))
            ...
            for h2 in self.net.hosts:
                if h == h2:
                    continue
                path = shortestpath.get(
                    h.name, h2.name, exclude=lambda n: n[0] == "h"
                )
                if not path:
                    continue
