...
if __name__ == "__main__":
    ...
    assert sp.get(1, 6) in [
        [1, 3, 6],
        [1, 5, 6],
    ]  # Multiple shortest paths from 1 to 6
    assert sp.get(6, 1) in [
        [6, 3, 1],
        [6, 5, 1],
    ]  # Multiple shortest paths from 6 to 1
    ...
    assert sp.get(1, 7) is None  # There is no path from node 1 to node 7
    assert sp.get(7, 2) is None  # There is no path from node 7 to node 2
