...
def configureP4Switch(**switch_args):
    ...
    if "sw_path" in switch_args and "grpc" in switch_args["sw_path"]:
        # If grpc appears in the BMv2 switch target,
        # we assume will start P4Runtime
...
class ExerciseTopo(Topo):
    ...
    def __init__(
        self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts
    ):
        ...
        for sw, params in switches.items():
            ...
            if "cpu_port" in params:
                self.addSwitch(
                    ...
                    cpu_port=params["cpu_port"],
                    cls=switchClass,
                )
            else:
                self.addSwitch(
                    sw, log_file="%s/%s.log" % (log_dir, sw), cls=switchClass
                )
...
class ExerciseRunner:
    ...
    Attributes:
        ...
        hosts    : dict<string, dict> // mininet host names
                                      // and their associated properties
        switches : dict<string, dict> // mininet switch names
                                      // and their associated properties
    ...
    def __init__(
        ...
        Arguments:
            topo_file : string    // A json file which describes the exercise's
                                     mininet topology.
            log_dir  : string     // Path to a directory
                                  // for storing exercise logs
    ...
    def program_switch_cli(self, sw_name, sw_dict):
        ...
        self.logger(
            "Configuring switch %s with file %s"
    ...
    def program_switches(self):
        ...
        for sw_name, sw_dict in self.switches.items():
            if "cli_input" not in sw_dict and "runtime_json" not in sw_dict:
                self.logger(
                    "Warning: No control plane file provided for switch %s."
                    % sw_name
                )
                continue
    ...
    def program_hosts(self):
        """Execute any commands provided in the"
        "topology.json file on each Mininet host"""
    ...
    def do_net_cli(self):
        ...
        print(
            "and your initial runtime configuration is loaded."
            "You can interact"
        )
        ...
        if "grpc" in self.bmv2_exe:
            ...
            print(
                " for example run:  cat %s/s1-p4runtime-requests.txt"
                % self.log_dir
            )
            print("")
