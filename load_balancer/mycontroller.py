...
def writeTableEntriesS1(p4info_helper, sw):
    ...
    # Egress
    upsertSendFrameEntry(
        p4info_helper=p4info_helper, sw=sw, egress_port=2, smac="00:00:00:01:02:00"
    )
    upsertSendFrameEntry(
        p4info_helper=p4info_helper, sw=sw, egress_port=3, smac="00:00:00:01:03:00"
    )


def upsertSendFrameEntry(p4info_helper, sw, egress_port, smac):
    table_entry = p4info_helper.buildTableEntry(
...
def deleteSendFrameEntry(p4info_helper, sw, egress_port):
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyEgress.send_frame",
        match_fields={"standard_metadata.egress_port": egress_port},
    )
    sw.WriteTableEntry(table_entry, update_type="DELETE")
    print(f"Deleted a 'send_frame' table entry on {sw.name=} with {egress_port=}")


def upsertEcmpNhopEntry(
    p4info_helper, sw, ecmp_select, dmac, ipv4, port, update_type="INSERT"
):
    table_entry = p4info_helper.buildTableEntry(
    ...
    print(
        f"Updated the 'ecmp_nhop' table on {sw.name=} with {ecmp_select=}, {ipv4=}, {dmac=}, {port=}"
    )


def deleteEcmpNhopEntry(p4info_helper, sw, ecmp_select):
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyIngress.ecmp_nhop", match_fields={"meta.ecmp_select": ecmp_select}
    )
    sw.WriteTableEntry(table_entry, update_type="DELETE")
...
def insert_hop():
    ...
    try:
        upsertEcmpNhopEntry(
            p4info_helper, s1, ecmp_select, dmac, ipv4, port, update_type="INSERT"
        )
...
def update_hop():
    ...
    try:
        upsertEcmpNhopEntry(
            p4info_helper, s1, ecmp_select, dmac, ipv4, port, update_type="MODIFY"
        )
        return jsonify({"status": "success"}), 200
    except grpc.RpcError as e:
        printGrpcError(e)
...
def delete_hop():
    ...
    try:
        deleteEcmpNhopEntry(p4info_helper, s1, ecmp_select)
        return jsonify({"status": "success"}), 200
    except grpc.RpcError as e:
        printGrpcError(e)
        return jsonify({"error": str(e)}), 500
