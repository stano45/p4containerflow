...
"""
This package contains several helper functions"
"for encoding to and decoding from byte strings:
- integers
...
def encodeNum(number, bitwidth):
    ...
    if orig_number < 0:
        print(
            "CONVERT_NEGATIVE_NUMBER debug: orig_number=%s"
            "number=%s bitwidth=%d num_str='%s'"
...
def encode(x, bitwidth):
    ...
    byte_len = bitwidthToBytes(bitwidth)
    if (type(x) is list or type(x) is tuple) and len(x) == 1:
        x = x[0]
    encoded_bytes = None
    if type(x) is str:
        if matchesMac(x):
